...
. if you have not done so already beforehand, you will need to install the required devevelopment tools and the sample application we prepared.
...
Together, the domain and acompanying representations are called a _studio_.
...
Select the _Woman_ _Entity_ and from the _Details_ view, select _Person_ as _Super Types_.
...
To developer more advanced features for your studios, which can not (yet) be implementated directly through the web UI, you can also do a lot of things programmatically (Java services, project templates, custom validation rules, representations descriptions...).
...
- Add the `PapayaOperationActivityLabelDetailToolContribution` (regsitration code in EditProjectview removed from the supplied version).
